...
async def cmd_start(message: Message, state: FSMContext):
    ...
    # Primera vez: bienvenida personalizada + reglas + botón
    name = f"@{message.from_user.username}" if message.from_user.username else message.from_user.first_name
    await message.answer(
        f"👋 ¡Bienvenido/a, *{name}*!\n\n"
        f"Bienvenido/a a *Underground FUT*.\n"
        f"Lee las reglas a continuación y acepta para empezar a jugar.\n\n"
        f"👋 Welcome, *{name}*!\n\n"
        f"Welcome to *Underground FUT*.\n"
        f"Read the rules below and accept to start playing.",
        parse_mode="Markdown"
    )
