...
# ── /pin — envía y ancla el mensaje de bienvenida en el grupo ────────────────
...
async def cmd_pin(message: Message, bot: Bot):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(
            text="▶️ Abrir Bot / Open Bot",
            url="https://t.me/Futelite_bot?start=welcome"
        )
    ]])
    ...
    pin_msg = await bot.send_message(
        GROUP_ID,
        "⚽ *UNDERGROUND FUT*\n\n"
        "Partidos de FC26 Ultimate Team con dinero real.\n"
        "Para jugar, abre el bot en privado y pulsa START.\n\n"
        "⚽ *UNDERGROUND FUT*\n\n"
        "FC26 Ultimate Team matches with real money.\n"
        "To play, open the bot in private and press START.",
        parse_mode="Markdown",
        reply_markup=kb
    )
    ...
    try:
        ...
        await message.answer("✅ Mensaje enviado y anclado en el grupo.")
    except Exception as e:
        await message.answer(
            f"✅ Mensaje enviado pero no se pudo anclar automáticamente.\n"
            f"Anclalo manualmente en el grupo."
        )

    db.log_audit("ADMIN_PIN_MESSAGE")
