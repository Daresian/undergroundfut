...
async def cb_i_paid(callback: CallbackQuery, bot: Bot):
    ...
    if not user or user["state"] != "WAITING_PAYMENT":
        await ac_trigger(bot, uid, "AC07", "Pulsó 'He pagado' sin pago pendiente válido")
        await callback.answer()
        return

    # Solo confirmar recepción — el admin es quien valida
    await callback.answer(
        "✅ Recibido. El admin validará tu pago en breve. / Received. The admin will validate your payment shortly.",
        show_alert=True
    )
